...
function testFairRotation(year: number, month: number) {
  ...
  // Let's track a rotation pointer across blocks so ties are broken in round-robin!
  let rrOffset = 0;
  ...
  blocks.forEach((block, bIdx) => {
    ...
    // Sort non-Karu nurses:
    // Primary: lowest pagiCount
    // Secondary: round-robin based on rrOffset
    const sortedNonKaru = [...nonKaruNurses].sort((a, b) => {
      const pDiff = (pagiCount[a.id] || 0) - (pagiCount[b.id] || 0);
      if (pDiff !== 0) return pDiff;
      const aIdx = (nonKaruNurses.findIndex(n => n.id === a.id) + rrOffset) % nonKaruNurses.length;
      const bIdx = (nonKaruNurses.findIndex(n => n.id === b.id) + rrOffset) % nonKaruNurses.length;
      return aIdx - bIdx;
    });
    ...
    rrOffset = (rrOffset + targetNonKaruPagi) % nonKaruNurses.length;
  });

  console.log(`\n=== Fair Rotation Results for ${year}-${month} (${daysInMonth} days, ${blocks.reduce((acc, b) => acc + b.length, 0)} workdays) ===`);
  nonKaruNurses.forEach((n) => {
    console.log(`${n.name.padEnd(35)}: P = ${pagiCount[n.id]}, S = ${siangCount[n.id]} (Diff: ${Math.abs(pagiCount[n.id] - siangCount[n.id])})`);
  });
}

testFairRotation(2026, 10);
testFairRotation(2026, 9);
testFairRotation(2026, 2);
testFairRotation(2026, 1);
